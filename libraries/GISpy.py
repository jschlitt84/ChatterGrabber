...
def sendCSV(cfg, directory,extra = ''):
    ...
    mailServer = smtplib.SMTP("smtp.gmail.com", 587, None, 120)
    mailServer.ehlo()
    mailServer.starttls()
    mailServer.ehlo()
...
def getReformatted(directory, lists, cfg, geoPickle, fileList,
                   core='null',
                   out_q='null',
                   keepTypes=['accepted','partial'],
                   NLPClassifier='null',
                   manualTime='null'):
    ...
    fileList = [fileRef for fileRef in fileList if getLoadType(directory,fileRef) != False]
...
def getOtherAPIs(directory, fileName='otherAPIs'):
    ...
    try:
        ...
        print "Could not log into the geocoder api. Location based filtering will fail for locations which have not been previously cached."
